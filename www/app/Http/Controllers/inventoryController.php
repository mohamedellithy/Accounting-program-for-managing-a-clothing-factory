...
class inventoryController extends Controller
{
    ...
    public function index()
    {
        //
        $product                = product::all();
        $finished_product       = product::Finished()->count();
        $least_product          = product::AlmostFinished()->count();
        $count_product_not_sale = order::distinct()->get();
        ...
        $Context                = [
            'all_reactionist'      =>$all_reactionist,
            'all_category'         =>$all_category,
            'all_ClothStyles'      =>$all_ClothStyles,
            'check_orderClothes'   =>$check_orderClothes,
            'cache_orderClothes'   =>$cache_orderClothes,
            'finished_orderClothes'=>$finished_orderClothes,
            'all_orderClothes'     =>$all_orderClothes,
            'product_not_sale'     =>$count_product_not_sale->count(),
            'least_product'        =>$least_product,
            'all_product'          =>$product->count(),
            'finished_product'     =>$finished_product,
        ];
        return view('admin.inventory.index')->with($Context);
    }
    ...
    public function datatableProducts(Request $request,$product_type){
        $product = product::all();
        if($product_type == 'finished'){
           $product = product::Finished()->get();
        }
        if($product_type == 'least'){
             $product = product::AlmostFinished()->get();
        }
        if($product_type == 'no-sale'){
            $get_all_product_ids = order::distinct()->pluck('product_id')->toArray();
            $product             = product::whereNotIn('id',$get_all_product_ids)->get();
        }
        return datatables()->of($product)
            ...
            ->addColumn('product_price',function($row){
                return $row->product_price.' جنيه ';
            })
            ->addColumn('full_price_handle',function($row){
                return $row->total_cost.' جنيه ';
            })
            ...
            ->rawColumns(['select','count_order','category','product_price','full_price_handle','count_piecies_handle','process','show'])->make(true);
    ...
    public function datatableorderClothes(Request $request,$order_type){
        ...
        return datatables()->of($orderClothes)
                ...
                ->addColumn('show', function($row) {
                        return '<a href='.url('orders-clothes/'.$row->id).' class="btn btn-success btn-sm">عرض </a>';
                    })
                ->rawColumns(['select','merchant_name','category_name','order_price','Quantity','process','show'])->make(true);
            ...
            ->addColumn('process', function($row) {
                 return '<div class="btn-group">
                            ...
                            <div class="dropdown-menu" role="menu">
                              ...
                              <div class="dropdown-divider"></div>
                              <a class="dropdown-item " href="'.url('piecies/'.$row->id.'/edit/'.$row->order_clothes_id).'"> <i class="fas fa-pencil-alt"></i>  تعديل </a>
                            </div>
                        </div>';
