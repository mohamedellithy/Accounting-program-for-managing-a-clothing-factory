	            ...
	            <div class="side-by-side col-md-8"> 
				   <div class="card" id="card_print_1">
		              <div class="card-header border-transparent">
		                <h3 class="card-title">طلبات القماش الخاصة بالتاجر <label > \ {{ get_merchant_name($merchant_id) }}</label> </h3>
		              </div>
		              <!-- /.card-header -->
				              	    ...
				              	    @foreach($order_clothes_info as $order_clothes)
					                    <tr>
					                      ...
					                      <td> {{ get_orderClothes_price($order_clothes->id) }} جنيه </td>
					                          @php $total_value+= get_orderClothes_price($order_clothes->id) @endphp
		              ...
		              <!--   <a href="{{ url('delete-merchant-orders/'.$merchant_id) }}"  class="btn btn-sm btn-info delete_all float-left" data-toggle="modal" data-target="#modal-default"  > <i class="far fa-trash-alt"></i> حذف الطلبات</a> -->
		                 <table style="width:100%">
                           	   <tr>
                           	      ...
                           	      <td> المبلغ المدفوع : {{ get_all_paid_from_merchant($merchant_id) }} جنيه </td>
                           	 
                           	      <td> المبلغ المتبقى : {{ ($total_value < get_all_paid_from_merchant($merchant_id))?'0 مديونية:'.(get_all_paid_from_merchant($merchant_id) - $total_value ):($total_value - get_all_paid_from_merchant($merchant_id)) }} جنيه  </td>
                           	    </tr> 
		            ...
		            <div class="card" id="card_print_2">
		              <div class="card-header border-transparent">
		                <h3 class="card-title">شيكات مطلوب تسديدها <label > \ {{ get_merchant_name($merchant_id) }}</label> </h3>
		              </div>
		              <!-- /.card-header -->
		              <div class="card-body p-0">
		                <div class="table-responsive" style="max-height: 400px;">
		                  <table class="table m-0">
		                    ...
		                    <tbody>
		              	        ...
		              	        @if(!empty($merchant_bankCheck))
			              	        @foreach($merchant_bankCheck as $bank_check)
		            ...
		            <div class="card" id="card_print_3">
		              <div class="card-header border-transparent" style="background-color:#CDDC39;" >
		                <h3 class="card-title">شيكات تم تسديدها  <label > \ {{ get_merchant_name($merchant_id) }}</label> </h3>
		              </div>
			              <!-- /.card-header -->
			              <div class="card-body p-0">
			                <div class="table-responsive" style="max-height: 400px;">
		            ...
		            <div class="card" id="card_print_4">
		              <div class="card-header border-transparent">
		                <h3 class="card-title">الدفعات الخاصة بالتاجر <label > \ {{ get_merchant_name($merchant_id) }}</label> </h3>
		              </div>
		              <!-- /.card-header -->
		              <div class="card-body p-0">
		                <div class="table-responsive" style="max-height: 400px;">
		                  <table class="table m-0">
		                    ...
		                    <tbody>
		                    	@if(!empty($merchant_postponed))
		                    	    @foreach($merchant_postponed as $postponed_data)
			                ...
			                <div class="table-responsive" style="max-height: 400px;">
      						   ...
      						   <div class="form-group col-12">
			                      ...
			                      <div class="form-group">
                                    ...
                                    @if( ( $total_value - get_all_paid_from_merchant($merchant_id) ) > 0):
                                       المبلغ المتبقى للتاجر : {{ $total_value - get_all_paid_from_merchant($merchant_id) }} جنيه
			                           <input name="postponed_value" type="text" class="form-control" placeholder="المبلغ المطلوب تسديدة ..." required="">
			                           <input name="order_price" value="{{ $total_value - get_all_paid_from_merchant($merchant_id) }}" type="text" class="form-control" placeholder="المبلغ المطلوب تسديدة ..." required="" hidden>
		           ...
		           <!-- here put check -->
                    <form method="post" action="{{ url('add-check-for-merchant/'.$merchant_id) }}">
                       {{ csrf_field() }}
                           @if( ( $total_value - get_all_paid_from_merchant($merchant_id) ) > 0): 
			                  ...
			                  <div class="checkform col-12" style="display:block">
	                            ...
	                            <div class="form-group col-md-12">
			                      ...
			                      <div class="form-group">
			                        ...
			                        <input name="check_owner" value="{{ get_merchant_name($merchant_id) }}" type="text" class="form-control" placeholder="شيك باسم ..." >
			                      </div>
			                    </div>
			                    <div class="form-group col-md-12">
			                      <!-- text input -->
